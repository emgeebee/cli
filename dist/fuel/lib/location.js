...
exports.normalizePostcode = exports.parseLocationInput = void 0;
const errors_1 = require("./errors");
const COORDINATE_PATTERN = /^\s*(?<latitude>-?(?:\d+(?:\.\d+)?))\s*,\s*(?<longitude>-?(?:\d+(?:\.\d+)?))\s*$/;
...
const parseLocationInput = (input) => {
    ...
    if (coordinateMatch?.groups) {
        ...
        if (!latitudeValue || !longitudeValue) {
            throw (0, errors_1.createAppError)("INVALID_INPUT", 'Expected a UK postcode like "SE1 9SG" or coordinates like "51.501,-0.141".');
        }
        const latitude = Number.parseFloat(latitudeValue);
        const longitude = Number.parseFloat(longitudeValue);
        if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90) {
            throw (0, errors_1.createAppError)("INVALID_INPUT", `Latitude "${latitudeValue}" is out of range.`);
        }
        if (!Number.isFinite(longitude) || longitude < -180 || longitude > 180) {
            throw (0, errors_1.createAppError)("INVALID_INPUT", `Longitude "${longitudeValue}" is out of range.`);
        }
        return {
            coordinates: {
                latitude,
    ...
    if (POSTCODE_PATTERN.test(trimmedInput)) {
        return {
            kind: "postcode",
            postcode: (0, exports.normalizePostcode)(trimmedInput)
        };
    }
    throw (0, errors_1.createAppError)("INVALID_INPUT", 'Expected a UK postcode like "SE1 9SG" or coordinates like "51.501,-0.141".');
};
exports.parseLocationInput = parseLocationInput;
...
exports.normalizePostcode = normalizePostcode;
