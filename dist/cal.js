...
function resolveCalConfig() {
    ...
    if (typeof token === "string" && token.trim() !== "") {
        const birthdayMonthDays = new Set();
        ...
        if (bdaySection) {
            for (const person of Object.values(bdaySection)) {
                ...
                if (!parsed)
                    continue;
                birthdayMonthDays.add(formatMonthDay(parsed.getUTCMonth(), parsed.getUTCDate()));
            }
        }
        return { token: token.trim(), birthdayMonthDays };
    }
    console.error(`Missing cal token at ${(0, config_1.getConfigPath)()} (expected config.cal.token).`);
    process.exit(1);
}
...
async function main() {
    ...
    printLegend();
    const allMonthLines = months.map(({ year, month }) => buildCalendarLines(year, month, today, colors));
    const gutter = "   ";
